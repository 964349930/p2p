...
<div class="tabs">
    <ul>
        <li class="current"><a href="javascript:void(0)" class="current">产品后台管理</a></li>
    </ul>
</div>
<div class="edit">
    <form method="post" action="<?php echo U('Admin/New/info');?>" enctype="multipart/form-data">
		...
		<dl>
            ...
            <?php if(!empty($info[cover])): ?><dd><img src="" height="50px"></dd><?php endif; ?>
		</dl>
		<dl>
            <dt></dt>
            <dd><textarea name="" cols="80" rows="4"><?php echo ($info["intro"]); ?></textarea></dd>
        </dl>
        <dl>
            <dt></dt>
            <dd><textarea name="" id="" cols="80" rows="4"><?php echo ($info["info"]); ?></textarea></dd>
        </dl>
        <dl>
            <dt></dt>
            <dd><select name="">
                <?php if(is_array($)): $i = 0; $__LIST__ = $;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($info[tpl_id]) == $vo[id]): ?>selected<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select></dd>
        <dl>
            <dt></dt>
